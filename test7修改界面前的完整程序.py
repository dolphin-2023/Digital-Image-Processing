...
from cupy.lib.stride_tricks import sliding_window_view as sliding_window_view2
...
# 优化前的卷积函数
def myconvolve2d(image, kernel, padding=0, stride=1):
    """
    自主实现的二维卷积函数,有点慢
    参数:
        image: 输入图像(二维或三维数组)
    ...
    for c in range(channels):
        for i in range(0, out_h):
            for j in range(0, out_w):
                ...
                # 提取图像块
                img_patch = padded[h_start:h_end, w_start:w_end, c]
                
                # 计算卷积结果
                output[i, j, c] = np.sum(img_patch * kernel)
...
def cupy_median_filter(image, kernel_size=3):
    ...
    for c in range(channels):
        # 创建滑动窗口视图 (形状: [H, W, K, K])
        windows = sliding_window_view2(padded[:, :, c], (kernel_size, kernel_size))
        
        # 计算每个窗口的中值 (axis=(2,3) 表示在最后两个维度上操作)
        output_gpu[:, :, c] = cp.median(windows, axis=(2, 3))
    ...
    print(f"cuda优化中值滤波函数运行时间：{end - start:.6f} 秒")
...
class ImageViewerApp:
    ...
    def _setup_ui(self):
        ...
        self._create_filter_controls()  # 新增滤波器控制区域
    ...
    def _create_filter_controls(self):
        ...
        # 滤波器选择框架
        filter_frame = ttk.LabelFrame(control_frame, text="图像滤波", padding=10)
        filter_frame.pack(side=tk.LEFT, padx=10, fill=tk.BOTH, expand=True)
        ...
        for text, command in filter_buttons:
            ttk.Button(filter_frame, text=text, command=command).pack(side=tk.LEFT, padx=5)
        ...
        self.kernel_size_slider.configure(command=self.update_kernel_size) # 给slider绑定事件
        ...
        self.sigma_slider.configure(command=self.update_sigma_value) # 给slider绑定事件
        # 分割线
