...
def mybincount(arr, minlength=None):
    ...
    np.add.at(counts, arr, 1) # 以arr为counts的索引，在对应位置加一
    return counts
    ...
    for i in range(pad, padded.shape[0]-pad):
        for j in range(pad, padded.shape[1]-pad):
            if connectivity == 8:
                ...
                neighborhood = np.zeros((kernel_size, kernel_size))
                neighborhood[pad, :] = padded[i, j-pad:j+pad+1]
                neighborhood[:, pad] = padded[i-pad:i+pad+1, j]
...
def connected_component_labeling(image, connectivity=8):
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            ...
            if connectivity == 8:
                if i > 0 and j > 0 and labeled[i-1, j-1] > 0:
                    neighbors.append(labeled[i-1, j-1])
                if i > 0 and j < image.shape[1]-1 and labeled[i-1, j+1] > 0:
                    neighbors.append(labeled[i-1, j+1])
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            if labeled[i, j] > 0:
                while labeled[i, j] in equivalences:
                    labeled[i, j] = equivalences[labeled[i, j]]
