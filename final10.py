...
def kmeans_segmentation(image, k, max_iters=100, tol=1e-4):
    ...
    if image.ndim == 2:
        ...
        h, w = image.shape
        channels = 1
        pixels = image.reshape((-1, 1)) # 重塑为 (N, 1) N = h*w
    elif image.ndim == 3:
        # 彩色图
        h, w, channels = image.shape
        if channels != 3:
            raise ValueError("彩色图像必须有3个通道 (RGB)")
        pixels = image.reshape((-1, channels)) # 重塑为 (N, C)，例如 (N, 3)
    else:
        raise ValueError("输入图像必须是2维（灰度）或3维（彩色）")

    # 将像素数据转换为 float32 以进行精确计算
    pixels = pixels.astype(np.float32)
    num_pixels = pixels.shape[0]

    # 2. 初始化簇中心 (Centroids)
    # 使用随机选择像素点作为初始中心
    # 为了避免重复，使用 random.sample
    initial_indices = random.sample(range(num_pixels), k)
    centroids = pixels[initial_indices]

    print(f"初始化完成，选择 {k} 个初始中心。")

    # 迭代变量
    labels = np.zeros(num_pixels, dtype=int) # 存储每个像素所属的簇标签

    # 3. 迭代优化
    for i in range(max_iters):
        print(f"迭代 {i+1}/{max_iters}...")
        old_centroids = centroids.copy()

        # 3.1 分配步骤 (Assignment Step)
        # 计算所有像素到所有簇中心的距离（欧氏距离的平方）
        # 使用广播机制加速计算：(N, 1, C) - (1, K, C) -> (N, K, C) -> (N, K)
        ...
        for cluster_idx in range(k):
            ...
            if len(cluster_pixels) > 0:
                ...
                # 处理空簇：可以选择重新随机初始化，或保持不变，或选择最远的点
                # 这里简单地保持不变（或可重新随机选点）
                # new_centroids[cluster_idx] = old_centroids[cluster_idx] # 保持不变
                # 或者重新随机选择一个点
                print(f"警告：簇 {cluster_idx} 在迭代 {i+1} 中变为空，重新随机初始化。")
        ...
        print(f"  中心点移动总距离: {centroid_shift:.4f}")
    ...
    # 4. 生成分割图像
    # 将每个像素的值替换为其所属簇的中心值
    # 使用最终的 labels 和 centroids
    segmented_pixels = centroids[labels]

    # 将结果重塑回原始图像的形状，并转换回 uint8
    ...
    end_time = time.time()
    print(f"K-Means 分割完成，耗时: {end_time - start_time:.2f} 秒。")
