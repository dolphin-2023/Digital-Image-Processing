...
def kmeans_segmentation(image, k, max_iters=100, tol=1e-4):
    """
    使用 K-Means 聚类算法对图像进行分割（向量化 K-Means++ 初始化）。

    参数:
        image (np.ndarray): 输入图像矩阵 (H, W) 或 (H, W, C)，数据类型通常为 uint8。
                            假设输入的彩色图像已经是 RGB 格式。
        k (int): 期望的聚类数量（分割区域数）。
        max_iters (int): 最大迭代次数。
        tol (float): 收敛阈值。当中心点移动的距离小于此值时停止迭代。

    返回:
        np.ndarray: 分割后的图像，每个像素被其所属簇的中心值替换。
                    与输入图像具有相同的尺寸和通道数，数据类型为 uint8。
    """
    print(f"开始 K-Means 分割 (向量化 K-Means++ 初始化)，k={k}...")
    ...
    if image.ndim == 2:
        ...
        h, w, channels = image.shape
        if channels != 3:
            raise ValueError("彩色图像必须有3个通道 (RGB)")
        pixels = image.reshape((-1, channels))
    else:
        raise ValueError("输入图像必须是2维（灰度）或3维（彩色）")

    pixels = pixels.astype(np.float32)
    num_pixels = pixels.shape[0]

    if k <= 0:
        raise ValueError("聚类数量 k 必须大于 0")
    if k > num_pixels:
        print(f"警告: k ({k}) 大于像素点数量 ({num_pixels}). 将 k 设置为 {num_pixels}.")
        k = num_pixels


    # 2. 初始化簇中心 (Centroids) - 使用向量化的 K-Means++
    print("执行向量化 K-Means++ 初始化...")
    start_time_init = time.time()
    centroids = np.zeros((k, channels), dtype=np.float32)

    # 2.1 选择第一个中心点：从所有像素中随机选择一个
    initial_idx = random.randint(0, num_pixels - 1)
    centroids[0] = pixels[initial_idx]
    print(f"  K-Means++: 第 1 个中心点选择完毕 (像素索引: {initial_idx})。")

    if k > 1: # 只有当 k > 1 时才需要选择后续中心点
        # `distances_to_closest_centroid_sq` 存储每个数据点到其最近的已选中心点的距离的平方
        # 初始化为无穷大
        distances_to_closest_centroid_sq = np.full(num_pixels, np.inf, dtype=np.float32)

        for i in range(1, k):
            # `latest_centroid` 是最新选出的中心点
            latest_centroid = centroids[i-1, np.newaxis, :] # 形状变为 (1, channels) 以便广播

            # --- 向量化计算开始 ---
            # 计算所有像素点到 `latest_centroid` 的距离平方
            # `pixels` 形状 (num_pixels, channels)
            # `latest_centroid` 形状 (1, channels)
            # `pixels - latest_centroid` 广播后形状 (num_pixels, channels)
            # `np.sum(..., axis=1)` 得到形状 (num_pixels,) 的距离平方数组
            dist_sq_to_latest_centroid = np.sum((pixels - latest_centroid)**2, axis=1)

            # 更新每个点到其最近的已选中心的距离平方
            # `np.minimum` 逐元素比较，取较小值
            distances_to_closest_centroid_sq = np.minimum(distances_to_closest_centroid_sq, dist_sq_to_latest_centroid)
            # --- 向量化计算结束 ---

            # 根据 D(x)^2 (即 distances_to_closest_centroid_sq) 的分布作为权重来选择下一个中心点
            sum_dist_sq = np.sum(distances_to_closest_centroid_sq)

            if sum_dist_sq == 0:
                # 处理所有点都与某个中心重合或距离为0的极端情况
                # 随机选择一个尚未被选为中心点的点（更鲁棒的方式是考虑那些不在已选中心列表中的点）
                # 为了简化，这里如果发生这种情况，我们从所有点中随机选，但应避免重复已选中心
                candidate_indices = np.arange(num_pixels)
                # 排除已经成为质心的点 (这步可能较慢，如果质心数量多)
                # 简单的做法：如果随机选到了已有的，就再选一次，或者直接忽略这个检查（理论上概率小）
                next_centroid_idx = random.choice(candidate_indices)
                # 更安全的做法：确保不重复，但会增加一点开销
                # chosen_pixel_indices = [np.where(np.all(pixels == c, axis=1))[0][0] for c in centroids[:i] if np.any(np.all(pixels == c, axis=1))]
                # available_indices = np.setdiff1d(candidate_indices, chosen_pixel_indices)
                # if len(available_indices) == 0: available_indices = candidate_indices # fallback
                # next_centroid_idx = random.choice(available_indices)
                print(f"  K-Means++: 注意，所有点到最近中心的距离平方和为0，随机选择下一个中心点。")
            else:
                probabilities = distances_to_closest_centroid_sq / sum_dist_sq
                # `np.random.choice` 会根据 p 参数指定的概率分布来选择元素
                next_centroid_idx = np.random.choice(num_pixels, p=probabilities)

            centroids[i] = pixels[next_centroid_idx]
            if i < 5 or i % 10 == 0 or i == k-1 : # 打印部分选择信息，避免过多输出
                 print(f"  K-Means++: 第 {i+1}/{k} 个中心点选择完毕 (像素索引: {next_centroid_idx})。")
    
    end_time_init = time.time()
    print(f"K-Means++ 初始化完成，耗时: {end_time_init - start_time_init:.2f} 秒。")
    ...
    # 3. 迭代优化
    print("开始 K-Means 迭代优化...")
    start_time_iter = time.time()
    for i in range(max_iters):
        # print(f"迭代 {i+1}/{max_iters}...") # 可以减少打印频率
        old_centroids = centroids.copy()

        # 3.1 分配步骤 (Assignment Step) - 向量化
        # `pixels` (N, C), `centroids` (K, C)
        # `pixels[:, np.newaxis, :]` -> (N, 1, C)
        # `centroids[np.newaxis, :, :]` -> (1, K, C)
        # `diff` -> (N, K, C)
        # `distances_sq_to_all_centroids` -> (N, K)
        distances_sq_to_all_centroids = np.sum((pixels[:, np.newaxis, :] - centroids[np.newaxis, :, :])**2, axis=2)
        labels = np.argmin(distances_sq_to_all_centroids, axis=1)

        # 3.2 更新步骤 (Update Step)
        # 使用 np.bincount 和 np.add.at 可以进一步向量化更新步骤，但分簇循环也相对清晰
        new_centroids = np.zeros_like(centroids)
        any_empty_cluster = False
        for cluster_idx in range(k):
            cluster_pixels = pixels[labels == cluster_idx]
            if len(cluster_pixels) > 0:
                new_centroids[cluster_idx] = np.mean(cluster_pixels, axis=0)
            else:
                any_empty_cluster = True
                # print(f"警告：簇 {cluster_idx} 在迭代 {i+1} 中变为空，重新随机初始化该中心。")
                # 重新随机初始化，并确保不选择已存在的中心点 (简化处理)
                new_centroids[cluster_idx] = pixels[random.choice(range(num_pixels))]
        if any_empty_cluster and (i < 5 or i % 10 == 0) : # 减少打印频率
            print(f"  迭代 {i+1}: 注意，出现空簇并已重新初始化。")
        ...
        if i < 5 or i % 10 == 0 or centroid_shift < tol: # 减少打印频率
            print(f"  迭代 {i+1}/{max_iters}, 中心点移动: {centroid_shift:.4f}")
        if centroid_shift < tol:
            print(f"算法在第 {i+1} 次迭代后收敛。")
            break

    if i == max_iters - 1:
        print(f"达到最大迭代次数 ({max_iters})。")
    end_time_iter = time.time()
    print(f"K-Means 迭代优化完成，耗时: {end_time_iter - start_time_iter:.2f} 秒。")

    # 4. 生成分割图像
    segmented_pixels = centroids[labels]
    segmented_image_shape = (h, w, channels) if channels > 1 else (h, w)
    segmented_image = segmented_pixels.reshape(segmented_image_shape)
    segmented_image = np.clip(segmented_image, 0, 255).astype(np.uint8)

    end_time_total = time.time()
    print(f"K-Means 分割总耗时: {end_time_total - start_time_total:.2f} 秒。")

    return segmented_image

def sklearn_kmeans_segmentation(image, k, max_iters=100, random_state=0):
    """
    使用 scikit-learn 的 KMeans 算法对图像进行分割。

    参数:
        image (np.ndarray): 输入图像矩阵 (H, W) 或 (H, W, C)，数据类型通常为 uint8。
        k (int): 期望的聚类数量（分割区域数）。
        max_iters (int): KMeans 算法的最大迭代次数。
    ...
    返回:
        np.ndarray: 分割后的图像，每个像素被其所属簇的中心值替换。
                    与输入图像具有相同的尺寸和通道数，数据类型为 uint8。
    """
    print(f"开始 Scikit-learn K-Means 分割，k={k}...")
    ...
    pixels = pixels.astype(np.float32) # KMeans 通常期望 float 类型输入

    # 2. 使用 scikit-learn 的 KMeans
    # n_init='auto' 会根据数据自动选择合适的初始化次数，推荐使用
    # tol 参数与你自定义函数中的 tol 类似，但这里使用默认值
    kmeans = KMeans(n_clusters=k, max_iter=max_iters, n_init='auto', random_state=random_state, tol=1e-4)
    kmeans.fit(pixels)

    # 获取簇标签和簇中心
    labels = kmeans.labels_
    centroids = kmeans.cluster_centers_ # 注意：sklearn 的 centroids 已经是 float 类型

    # 3. 生成分割图像
    segmented_pixels = centroids[labels]
    segmented_image = segmented_pixels.reshape((h, w, channels) if channels > 1 else (h, w))
    segmented_image = np.clip(segmented_image, 0, 255).astype(np.uint8)

    end_time = time.time()
    print(f"Scikit-learn K-Means 分割完成，耗时: {end_time - start_time:.2f} 秒。")
...
class ImageViewerApp:
    ...
    def _create_segmentation_controls(self, parent):
        ...
        # K-Means分割控制
        kmeans_frame = ttk.Frame(frame)
        kmeans_frame.pack(side=tk.LEFT, padx=10, pady=5, fill=tk.X) # 让K-Means控件靠左排列
        # K值静态标签
        ttk.Label(kmeans_frame, text="K值:").pack(side=tk.LEFT, padx=(5, 0))
        # 初始化 K 值变量
        self.kmeans_k_value = 3 # 默认 K=3
        # 显示 K 值的标签
        self.kmeans_k_label = ttk.Label(kmeans_frame, text=str(self.kmeans_k_value))
        self.kmeans_k_label.pack(side=tk.LEFT, padx=(0,10)) # 稍微增加右边距
        # 调整K值的滑动条
        self.kmeans_k_slider = ttk.Scale(
            kmeans_frame, from_=2, to=16, # K值范围，2到16
            orient=tk.HORIZONTAL, length=100,
            command=self.update_kmeans_k_label # 拖动时调用更新标签的函数
        )
        self.kmeans_k_slider.set(self.kmeans_k_value) # 设置初始值
        self.kmeans_k_slider.pack(side=tk.LEFT, padx=5)

        # K-Means 分割按钮
        ttk.Button(kmeans_frame, text="自定义K-Means分割", command=self.apply_kmeans_segmentation, style='Tool.TButton').pack(side=tk.LEFT, padx=5)
        # Sklearn K-Means 分割按钮
        ttk.Button(kmeans_frame, text="Sklearn K-Means", command=self.apply_sklearn_kmeans_segmentation, style='Tool.TButton').pack(side=tk.LEFT, padx=5)
    ...
    def _create_info_display(self, parent):
        ...
        self.figure = Figure(figsize=(10, 2.5)) # 10英寸宽，2.5英寸高
    ...
    '''
    def update_processed_display(self):
        """更新处理后图像显示 (使其行为与 original_display 类似)"""
        if self.processed_image is None:
            if hasattr(self, 'image_canvas_right') and self.image_canvas_right:
                self.image_canvas_right.delete("all")  # 清空画布
                self.image_canvas_right.image = None    # 清除引用
            return
            
        try:
            pil_image = create_pil_image(self.processed_image)
            canvas = self.image_canvas_right # 注意这里是 image_canvas_right
            ...
            # 防止除零或尺寸无效
            if canvas_width <= 1 or canvas_height <= 1: # 使用1作为最小有效尺寸
                # print("Warning: Processed canvas dimensions are too small or not yet determined. Using default.")
                canvas_width = 200 # 设定一个默认最小尺寸
                canvas_height = 200
            if pil_image.width <= 0 or pil_image.height <= 0:
                print("Error: Processed PIL image has zero or negative dimension.")
                # 清空画布并返回，如果图像无效
                canvas.delete("all")
                canvas.image = None
                return
            ...
            # 确保 img_width 和 img_height 是正数
            if img_width <= 0 or img_height <= 0:
                print(f"Error: Invalid image dimensions for processed image ({img_width}x{img_height}).")
                canvas.delete("all")
                canvas.image = None
                return
            ...
            # 使用高质量的 LANCZOS 采样进行缩放
            resized_image = pil_image.resize(new_size, Image.Resampling.LANCZOS)

            tk_image = ImageTk.PhotoImage(resized_image)
            
            canvas.delete("all") # 清空之前的图像
            canvas.create_image(
                canvas_width // 2,   # 在画布中心x坐标
                canvas_height // 2,  # 在画布中心y坐标
                image=tk_image,
                anchor=tk.CENTER     # 将图像的中心对准指定的坐标
            )
            canvas.image = tk_image  # 保持对tk_image的引用，防止被垃圾回收

            # 移除了原有的 scrollregion, config(width/height), 和 xview_moveto/yview_moveto
            # 因为现在图像是直接缩放到适应Canvas大小

        except Exception as e:
            print(f"Error during update_processed_display: {e}")
            import traceback
            traceback.print_exc() # 打印详细错误信息
            # 出错时尝试清空画布
            if hasattr(self, 'image_canvas_right') and self.image_canvas_right:
                self.image_canvas_right.delete("all")
                self.image_canvas_right.image = None
    ...
    def apply_kmeans_segmentation(self):
        ...
        # 从实例变量 self.kmeans_k_value 获取当前 K 值 (由滑动条更新)
        k = self.kmeans_k_value

        try:
            print(f"准备使用 K={k} 进行 K-Means 分割...") # 添加调试信息
            # 调用你之前添加的 K-Means 分割函数
            # 假设函数名为 kmeans_segmentation
            segmented = kmeans_segmentation(self.original_image, k=k) # 使用从滑动条获取的 k 值

            # 更新处理后的图像显示
            self._update_processed_image(segmented)
            print("K-Means 分割完成并已更新显示。") # 添加调试信息

        except Exception as e:
            messagebox.showerror("K-Means 错误", f"分割过程中出现错误: {str(e)}")
            import traceback
            traceback.print_exc() # 在控制台打印详细错误信息，方便调试

    def apply_sklearn_kmeans_segmentation(self):
        """应用 Scikit-learn 的 K-Means 分割"""
        if self.original_image is None:
            messagebox.showwarning("提示", "请先打开图片")
            return
        ...
        # 从实例变量 self.kmeans_k_value 获取当前 K 值 (由滑动条更新)
        k = self.kmeans_k_value
        # 你也可以考虑为 sklearn 版本单独获取迭代次数等参数，但这里我们简化处理
        # max_iters_sklearn = simpledialog.askinteger("Sklearn K-Means", "请输入最大迭代次数:",
        #                                             parent=self.master, minvalue=10, maxvalue=1000, initialvalue=100)
        # if max_iters_sklearn is None:
        #     return

        try:
            print(f"准备使用 K={k} 进行 Scikit-learn K-Means 分割...")
            # 调用你新增的 sklearn K-Means 分割函数
            segmented = sklearn_kmeans_segmentation(self.original_image, k=k) # 可以传入 max_iters_sklearn 如果需要

            # 更新处理后的图像显示
            self._update_processed_image(segmented)
            print("Scikit-learn K-Means 分割完成并已更新显示。")

        except NameError as e: # 捕获 KMeans 未定义的错误
             if 'KMeans' in str(e):
                 messagebox.showerror("依赖错误", "KMeans 未定义。请确保已安装 scikit-learn 并在代码顶部正确导入：\nfrom sklearn.cluster import KMeans")
             else:
                 messagebox.showerror("Sklearn K-Means 错误", f"分割过程中出现错误: {str(e)}")
             import traceback
             traceback.print_exc()
        except Exception as e:
            messagebox.showerror("Sklearn K-Means 错误", f"分割过程中出现错误: {str(e)}")
    ...
    def update_kmeans_k_label(self, value):
        """更新 K-Means 的 K 值和标签显示"""
        # value 是滑动条传来的浮点数值
        self.kmeans_k_value = int(float(value)) # 四舍五入或直接取整
        self.kmeans_k_label.config(text=str(self.kmeans_k_value))
